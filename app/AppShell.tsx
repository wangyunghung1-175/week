...
export default function AppShell({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    setTab(window.location.pathname.replace('/', '') || 'weekly');
    ...
    <AuthCtx.Provider value={{ user, loading }}>
      ...
      <header style={{ background:'var(--primary)', color:'white', padding:'0 16px', height:'52px', display:'flex', alignItems:'center', justifyContent:'space-between', position:'sticky', top:0, zIndex:100, boxShadow:'0 2px 8px rgba(0,0,0,.2)' }}>
        <div>
          ...
          <div style={{ fontSize:'10px', opacity:.6 }}>{user.user_metadata?.display_name ?? user.email ?? ''}</div>
        </div>
        <button onClick={signOut} style={{ background:'rgba(255,255,255,.12)', border:'1px solid rgba(255,255,255,.25)', color:'white', borderRadius:'6px', padding:'5px 12px', fontSize:'12px', cursor:'pointer' }}>
          登出
        </button>
      </header>
      ...
      <nav style={{ background:'var(--primary)', borderTop:'1px solid rgba(255,255,255,.1)', display:'flex', overflowX:'auto' }}>
        {TABS.map(t => (
          <button key={t.id} onClick={() => handleNav(t.id, t.href)}
            style={{ padding:'9px 18px', background:'none', border:'none', cursor:'pointer', fontFamily:'inherit', fontSize:'13px', fontWeight:500, whiteSpace:'nowrap', transition:'all .15s',

...
import { createClient, CORE_GOALS, GOAL_COLORS, getWeekDates, formatDate } from '../../lib/supabase';
...
export default function TeamPage() {
  ...
  const [monthVal,   setMonthVal]   = useState(() => { const n=new Date(); return `${n.getFullYear()}-${String(n.getMonth()+1).padStart(2,'0')}`; });
  ...
  const buildFilter = () => {
    ...
    for (let w=1;w<=53;w++) {
      ...
      try { const {mon,fri}=getWeekDates(wk); if((mon.getFullYear()==+yr&&mon.getMonth()+1==+mo)||(fri.getFullYear()==+yr&&fri.getMonth()+1==+mo)) wks.push(wk); } catch {break;}
    }
  ...
  const load = async () => {
    ...
    try {
      ...
      const [sr, ir] = await Promise.all([
        h.from('weekly_summary').select('*').or(f.replace('=','eq.').replace('like.','like.')).order('user_name').order('week_key'),
        h.from('work_items').select('*').order('user_name').order('week_key'),
      ]);
      // Re-filter items to match period
      // Simpler: use raw fetch with query string
      const headers = { apikey: process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!, Authorization: `Bearer ${process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!}`, 'Content-Type':'application/json' };
      const base = process.env.NEXT_PUBLIC_SUPABASE_URL + '/rest/v1';
      const [sResp, iResp] = await Promise.all([
        fetch(`${base}/weekly_summary?${f}&order=user_name,week_key`, { headers }),
        fetch(`${base}/work_items?${f}&order=user_name,week_key`,     { headers }),
      ]);
  ...
  const exportExcel = () => {
    ...
    // Sheet 1: summary
    const sumData = [['成員','週別','應工時','實際工時','休假','加班','交通','差異'], ...summary.map(r => [r.user_name,r.week_key,r.expected_hours,r.actual_hours,r.leave_hours,r.ot_hours,r.traffic_hours,r.diff_hours])];
    XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet(sumData), '週報摘要');
    // Sheet 2: items
    const itemData = [['成員','週別','核心目標','中分類','項目分類','工作事項','時數','備註'], ...items.map(r=>[r.user_name,r.week_key,r.goal,r.subcat,r.category,r.item_name,r.hours,r.note])];
    XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet(itemData), '工作項目明細');
    XLSX.writeFile(wb, `雙福團隊週報_${monthVal||weekVal||yearVal}.xlsx`);
  };

  const members: Record<string,{actual:number;expected:number;leave:number;ot:number;weeks:number}> = {};
  summary.forEach(r => {
    if (!members[r.user_name]) members[r.user_name] = {actual:0,expected:0,leave:0,ot:0,weeks:0};
    const m = members[r.user_name];
    m.actual   += +r.actual_hours||0;
    m.expected += +r.expected_hours||0;
    m.leave    += +r.leave_hours||0;
    m.ot       += +r.ot_hours||0;
    m.weeks++;
  });
  const memberNames = Object.keys(members).sort().filter(n => !search || n.toLowerCase().includes(search.toLowerCase()));
  ...
  items.forEach(r => { if (r.goal && goalTotals[r.goal]!==undefined) goalTotals[r.goal]+=+r.hours||0; });
  ...
  return (
    <div>
      ...
      <div style={{ background:'white', border:'1px solid var(--border)', borderRadius:'var(--radius)', padding:'12px 16px', marginBottom:'14px', display:'flex', alignItems:'center', gap:'10px', flexWrap:'wrap', boxShadow:'var(--shadow)' }}>
        ...
        {periodType==='year'  && <select value={yearVal} onChange={e=>setYearVal(e.target.value)} style={{ padding:'6px 10px', border:'1px solid var(--border)', borderRadius:'6px', fontSize:'13px', fontFamily:'inherit' }}><option value="2024">2024</option><option value="2025">2025</option><option value="2026">2026</option></select>}
        <button onClick={load} disabled={loading}
          style={{ padding:'6px 16px', background:'var(--primary)', color:'white', border:'none', borderRadius:'6px', cursor:loading?'default':'pointer', fontSize:'13px', fontWeight:600, fontFamily:'inherit', opacity:loading?0.7:1 }}>
          {loading ? '載入中…' : '🔄 載入資料'}
        </button>
        <button onClick={exportExcel} disabled={!summary.length}
          style={{ padding:'6px 14px', background:'rgba(46,139,87,0.1)', color:'var(--success)', border:'1px solid rgba(46,139,87,0.25)', borderRadius:'6px', cursor:summary.length?'pointer':'default', fontSize:'12px', fontWeight:600, fontFamily:'inherit', opacity:summary.length?1:0.5 }}>⬇ 匯出Excel</button>
      </div>
      ...
      {summary.length > 0 && (
        <>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(120px,1fr))', gap:'8px', marginBottom:'14px' }}>
            {[
              ['填報人數', Object.keys(members).length, ''],
              ['涵蓋週數', [...new Set(summary.map(r=>r.week_key))].length, ''],
              ['團隊總工時', totalActual.toFixed(0), 'accent'],
              ['人均工時', Object.keys(members).length ? (totalActual/Object.keys(members).length).toFixed(1) : 0, ''],
          ...
          <div style={{ background:'white', border:'1px solid var(--border)', borderRadius:'var(--radius)', marginBottom:'14px', boxShadow:'var(--shadow)', overflow:'hidden' }}>
            ...
            <div style={{ padding:'14px 16px' }}>
              {Object.keys(members).map((name, i) => {
                const m = members[name];
                const pct = m.actual/Math.max(...Object.values(members).map(x=>x.actual),1)*100;
                const expPct = m.expected/Math.max(...Object.values(members).map(x=>x.actual),1)*100;
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'14px', marginBottom:'14px' }}>
            <div style={{ background:'white', border:'1px solid var(--border)', borderRadius:'var(--radius)', boxShadow:'var(--shadow)', overflow:'hidden' }}>
              ...
              <div style={{ padding:'14px 16px' }}>
                {CORE_GOALS.map(g => {
                  const h = goalTotals[g], pct=(h/totalGoal*100).toFixed(1);
            ...
            <div style={{ background:'white', border:'1px solid var(--border)', borderRadius:'var(--radius)', boxShadow:'var(--shadow)', overflow:'hidden' }}>
              ...
              <div style={{ padding:'4px 0' }}>
                <table style={{ width:'100%', borderCollapse:'collapse', fontSize:'12px' }}>
                  ...
                  <tbody>{Object.keys(members).map((n,i)=>{
                    const m=members[n];
                    return <tr key={n} style={{ background:i%2?'var(--surface2)':'white' }}>
          ...
          <div style={{ background:'white', border:'1px solid var(--border)', borderRadius:'var(--radius)', boxShadow:'var(--shadow)', overflow:'hidden' }}>
            ...
            <div style={{ overflowX:'auto' }}>
              <table style={{ width:'100%', borderCollapse:'collapse', fontSize:'13px' }}>
                <thead><tr style={{ background:'var(--primary)', color:'white' }}>
                  {['成員','週別','應工時','實際工時','休假','加班','差異','狀態'].map(h=><th key={h} style={{ padding:'8px 12px', textAlign:h==='成員'||h==='週別'?'left':'center', fontSize:'12px' }}>{h}</th>)}
                </tr></thead>
                <tbody>
                  {summary.filter(r=>!search||r.user_name.toLowerCase().includes(search.toLowerCase())).map((r,i)=>{
                    const diff = +r.diff_hours||0;
                    return <tr key={r.id||i} style={{ borderBottom:'1px solid var(--border)', background:i%2?'var(--surface2)':'white' }}>
                      ...
                      <td style={{ padding:'7px 12px', textAlign:'center', fontFamily:'DM Mono' }}>{(+r.actual_hours||0).toFixed(1)}</td>
                      <td style={{ padding:'7px 12px', textAlign:'center', fontFamily:'DM Mono' }}>{(+r.leave_hours||0).toFixed(1)}</td>
                      <td style={{ padding:'7px 12px', textAlign:'center', fontFamily:'DM Mono' }}>{(+r.ot_hours||0).toFixed(1)}</td>
                      ...
                      <td style={{ padding:'7px 12px', textAlign:'center' }}>
                        <span style={{ display:'inline-block', padding:'2px 8px', borderRadius:'20px', fontSize:'11px', fontWeight:600, background:(+r.actual_hours||0)>0?'rgba(46,139,87,0.1)':'rgba(192,57,43,0.1)', color:(+r.actual_hours||0)>0?'var(--success)':'var(--danger)' }}>
                          {(+r.actual_hours||0)>0?'已填報':'未填時數'}
                        </span>
                      </td>
                    </tr>;
                  })}
